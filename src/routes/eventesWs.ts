...
const startWebSocketServer = (io: { on: (arg0: string, arg1: (socket: any) => void) => void; }) => {
 
  const router = express.Router();
io.on("connection", (socket) => {
  console.log("New WebSocket connection");

  socket.on("getFeatures", async () => {
  try {
    ...
    // socket.emit("featuresResponse", featuresWithType );
    

    socket.send(featuresWithType);

  } catch (err) {
    socket.emit("featuresError", err);
  }
})
});
// return server
